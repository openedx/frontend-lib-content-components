...
describe('Editor', () => {
  describe('snapshots', () => {
    test('renders no editor when ref isnt ready', () => {
        prepareEditorRefSpy.mockImplementationOnce((args)=>({editorRef: null, refReady: false, setEditorRef: jest.fn()}));
        expect(shallow(<Editor {...props}
        />)).toMatchSnapshot();
      });
      test('renders editor when ref is ready', () => {
        prepareEditorRefSpy.mockImplementationOnce((args)=>({editorRef: {current:'ref'}, refReady: true, setEditorRef: jest.fn().mockName('setEditorRef')}));
        expect(shallow(<Editor {...props}
        />)).toMatchSnapshot();
      });
      test('presents error message if no relevant editor found and ref ready', () => {
        prepareEditorRefSpy.mockImplementationOnce((args)=>({editorRef: {current:'ref'}, refReady: true, setEditorRef: jest.fn().mockName('setEditorRef')}));
        expect(shallow(<Editor {...props} blockType={'fAkEBlock'}
        />)).toMatchSnapshot();
      });
  });
